...
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface, publicDescription string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to create asset JSON: %w", err)
	}

	err = ctx.GetStub().PutState(assetID, assetBytes)
	if err != nil {
		return "", fmt.Errorf("failed to put asset in public data: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("failed setting state based endorsement for buyer and seller: %w", err)
	}
...
func (s *SmartContract) VerifyAssetProperties(ctx contractapi.TransactionContextInterface, assetID string) (bool, error) {
	transMap, err := ctx.GetStub().GetTransient()
	if err != nil {
		return false, fmt.Errorf("error getting transient: %w", err)
	}
	...
	asset, err := s.ReadAsset(ctx, assetID)
	if err != nil {
		return false, fmt.Errorf("failed to get asset: %w", err)
	}
	...
	if err != nil {
		return false, fmt.Errorf("failed to read asset private properties hash from seller's collection: %w", err)
	}
...
func (s *SmartContract) TransferAsset(ctx contractapi.TransactionContextInterface, assetID string, buyerOrgID string) error {
	...
	transMap, err := ctx.GetStub().GetTransient()
	if err != nil {
		return fmt.Errorf("error getting transient data: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to unmarshal price JSON: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed asset transfer: %w", err)
	}
...
func verifyTransferConditions(ctx contractapi.TransactionContextInterface,
	...
	// CHECK2: Verify that buyer and seller on-chain asset definition hash matches
	...
	if err != nil {
		return fmt.Errorf("failed to read asset private properties hash from seller's collection: %w", err)
	}
	...
	// verify that buyer and seller on-chain asset definition hash matches
	...
	if err != nil {
		return fmt.Errorf("failed to get seller price hash: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to get buyer price hash: %w", err)
	}
...
func transferAssetState(ctx contractapi.TransactionContextInterface, asset *Asset, clientOrgID string, buyerOrgID string, price int) error {
	...
	if err != nil {
		return fmt.Errorf("failed to write asset for buyer: %w", err)
	}
	...
	err = setAssetStateBasedEndorsement(ctx, asset.ID, endorsingOrgs)
	if err != nil {
		return fmt.Errorf("failed setting state based endorsement for new owner: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to delete Asset private details from seller: %w", err)
	}

	// Delete the price records for seller
	assetPriceKey, err := ctx.GetStub().CreateCompositeKey(typeAssetForSale, []string{asset.ID})
	if err != nil {
		return fmt.Errorf("failed to create composite key for seller: %w", err)
	}
	err = ctx.GetStub().DelPrivateData(collectionSeller, assetPriceKey)
	if err != nil {
		return fmt.Errorf("failed to delete asset price from implicit private data collection for seller: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to create composite key for buyer: %w", err)
	}
	err = ctx.GetStub().DelPrivateData(collectionBuyer, assetPriceKey)
	if err != nil {
		return fmt.Errorf("failed to delete asset price from implicit private data collection for buyer: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to create composite key for receipt: %w", err)
	}

	txTimestamp, err := ctx.GetStub().GetTxTimestamp()
	if err != nil {
		return fmt.Errorf("failed to create timestamp for receipt: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to put private asset receipt for buyer: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to put private asset receipt for seller: %w", err)
	}
...
func getClientOrgID(ctx contractapi.TransactionContextInterface) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed getting client's orgID: %w", err)
	}
...
func verifyClientOrgMatchesPeerOrg(clientOrgID string) error {
	...
	if err != nil {
		return fmt.Errorf("failed getting peer's orgID: %w", err)
	}
...
func setAssetStateBasedEndorsement(ctx contractapi.TransactionContextInterface, assetID string, orgsToEndorse []string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to set validation parameter on asset: %w", err)
	}
...
func (s *SmartContract) GetAssetHashID(ctx contractapi.TransactionContextInterface) (string, error) {
	...
	asset, err := s.ReadAsset(ctx, assetID)
	if err != nil {
		return "", fmt.Errorf("failed to get asset: %w, asset properties provided do not represent any on chain asset", err)
	}
	if asset.ID != assetID {
		return "", errors.New("Asset properties provided do not correspond to any on chain asset")
	}
	return asset.ID, nil
}
