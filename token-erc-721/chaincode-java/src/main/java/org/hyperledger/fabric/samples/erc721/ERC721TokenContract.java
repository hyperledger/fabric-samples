...
@Contract(name = "erc721token", info = @Info(title = "ERC721Token Contract", description = "The erc721 NFT implementation", version = "0.0.1-SNAPSHOT", license = @License(name = "Apache 2.0 License", url = "http://www.apache.org/licenses/LICENSE-2.0.html"), contact = @Contact(email = "renjith.narayanan@gmail.com", name = "Renjith Narayanan", url = "https://hyperledger.example.com")))
@Default
public class ERC721TokenContract implements ContractInterface {
  ...
  public void approve(final Context ctx, final String operator, final String tokenId) {
    ...
    if ((!owner.equalsIgnoreCase(sender)) && (!operatorApproval)) {
      final String errorMessage = String.format(
          ...
          sender, tokenId);
      throw new ChaincodeException(errorMessage, ContractErrors.UNOTHERIZED_SENDER.toString());
    }
   ...
   */
  @Transaction(intent = Transaction.TYPE.EVALUATE)
  public String getSymbol(final Context ctx) {
   ...
   */
  @Transaction(intent = Transaction.TYPE.EVALUATE)
  public String getTokenURI(final Context ctx, final String tokenId) {
    final NFT nft = this.readNft(ctx, tokenId);
