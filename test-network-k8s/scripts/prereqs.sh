...
function check_prereqs() {
  ...
  FABRIC_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm ${FABRIC_PEER_IMAGE} peer version | sed -ne 's/^ Version: v\?\(.*\)$/\1/p')
  FABRIC_CA_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm ${FABRIC_CONTAINER_REGISTRY}/fabric-ca:$FABRIC_CA_VERSION fabric-ca-server version | sed -ne 's/^ Version: v\?\(.*\)$/\1/p')
  ...
  FABRIC_LOCAL_VERSION=$(bin/peer version | sed -ne 's/^ Version: v\?\(.*\)$/\1/p')
  FABRIC_CA_LOCAL_VERSION=$(bin/fabric-ca-client version | sed -ne 's/^ Version: v\?\(.*\)$/\1/p')
